...
for target_file in files:
    if target_file.endswith('_filtered.vot'):  # Check for specific file 
        ...
        # Initialize an array to store the l1 index for each object
        ...
        # Extract valid magnitudes (remove NaN)
            ...
            if len(valid_mags) > 1:  # Perform calculation only if there are at least 2 values
                ...
                #sum of (m_i - mean) * (m_{i+1} - mean)
                numerator = np.sum((valid_mags[:-1] - mean_magnitude[i]) * (valid_mags[1:] - mean_magnitude[i]))
                #sum of (m_i - mean)^2
                denominator = np.sum((valid_mags - mean_magnitude[i]) ** 2)
