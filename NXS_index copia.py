    ...
    if target_file.endswith('.vot'):  # Check for specific file 
        ...
        for i in range(mag_matrix.shape[0]):
            ...
            if len(valid_mags) > 1:
                ...
                variance_term = np.sum((valid_mags - weighted_mag[i])**2 - valid_mager**2)
                
                # NXS calculation as per formula
                nxs[i] = variance_term / (len(valid_mags) * weighted_mag[i]**2)
        ...
        new_table = Table(data=[ra_column, dec_column, N, nxs, mean_magnitude], names=('RA', 'DEC', 'N', 'excess_variance', 'mean_magnitude'))
